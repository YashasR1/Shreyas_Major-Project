...
// Dynamic 3-day window: Present Date (Today), Tomorrow (+1 Day), and Day After Tomorrow (+2 Days)
const generateDynamicSlots = () => {
  const slotsList = [];
  const sessions = [
    { idSuffix: 'morn', hours: 10, minutes: 0, cap: 25, booked: 6 },
    { idSuffix: 'aftn', hours: 14, minutes: 30, cap: 25, booked: 18 },
    { idSuffix: 'even', hours: 17, minutes: 0, cap: 20, booked: 11 },
  ];
  ...
  [0, 1, 2].forEach((dayOffset) => {
    sessions.forEach((session) => {
      ...
      d.setHours(session.hours, session.minutes, 0, 0);

      slotsList.push({
        id: `slot-d${dayOffset}-${session.idSuffix}`,
        day_offset: dayOffset,
        slot_time: d.toISOString(),
        max_capacity: session.cap,
        current_bookings: session.booked,
      });
    });
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (slotError || !slotData || slotData.length === 0) {
        ...
        // Dynamically project database slots into Today, Tomorrow, and Day After Tomorrow
        const mappedSlots = slotData.map((dbSlot, index) => {
          const dayOffset = index % 3; // 0 = Today, 1 = Tomorrow, 2 = Day After Tomorrow
          const sessionHour = 10 + (Math.floor(index / 3) % 3) * 3;
          const d = new Date();
          d.setDate(d.getDate() + dayOffset);
          d.setHours(sessionHour, 0, 0, 0);
        ...
        setSlots(mappedSlots.length > 0 ? mappedSlots : dynamicSlots);
      }
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 shadow-sm">
        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2.5">
          ...
          <span>{t('slotReservationTitle', 'Slot Reservation')}</span>
        </h2>
      </div>
      ...
      {myBookings.length > 0 && (
        <div className="bg-blue-50/50 border border-blue-100 rounded-3xl p-6 shadow-sm">
          <h3 className="font-bold text-gray-900 text-sm mb-3 flex items-center gap-2">
            ...
            <span>{t('reservedSlotsTitle', 'Your Reserved Pickup Slots')}</span>
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {myBookings.map((book) => (
              <div key={book.id} className="p-3.5 rounded-xl bg-white border border-blue-200 flex items-center justify-between shadow-sm">
                <div>
                  <span className="text-xs font-mono text-blue-600 font-bold uppercase">{t('guaranteedReservation', 'GUARANTEED RESERVATION')}</span>
                  <div className="text-sm font-bold text-gray-900 mt-0.5">
                    {new Date(book.slot_time).toLocaleDateString(getLocaleTag(), { weekday: 'short', month: 'short', day: 'numeric' })} at {new Date(book.slot_time).toLocaleTimeString(getLocaleTag(), { hour: '2-digit', minute: '2-digit' })}
                  </div>
                </div>
                <span className="px-3 py-1 rounded-lg bg-blue-100 text-blue-800 font-bold text-xs border border-blue-200">
                  {t('confirmedBadge', 'CONFIRMED')}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-gray-900 flex items-center justify-between">
          <span>{t('shopSlotsTitle', 'Shop Distribution Slots')}</span>
        </h3>

        {loading ? (
          <div className="p-12 text-center text-gray-500 flex flex-col items-center justify-center gap-3">
            <Loader2 className="w-8 h-8 animate-spin text-blue-600" />
            <span className="text-sm font-mono">{t('checkingCapacities', 'Checking database capacities...')}</span>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {slots.map((slot) => {
              ...
              const isAlreadyBooked = myBookings.some(b => b.slot_id === slot.id || b.slot_time === slot.slot_time);

              // Calculate relative day badges (Today, Tomorrow, Day After Tomorrow)
              ...
              const dayBadge = isToday
                ? { text: t('todayBadge', 'TODAY'), bg: 'bg-emerald-100 text-emerald-800 border-emerald-300' }
                : isTomorrow
                ? { text: t('tomorrowBadge', 'TOMORROW'), bg: 'bg-blue-100 text-blue-800 border-blue-300' }
                : { text: t('dayAfterBadge', 'DAY AFTER TOMORROW'), bg: 'bg-purple-100 text-purple-800 border-purple-300' };
                ...
                <div key={slot.id} className={`bg-white rounded-3xl p-6 shadow-sm border transition-all flex flex-col justify-between ${isFull ? 'opacity-60 bg-gray-50 border-gray-200' : 'border-gray-200 hover:shadow-md hover:border-blue-200'}`}>
                  <div>
                    <div className="flex items-center justify-between text-xs font-mono mb-2">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase border ${dayBadge.bg}`}>
                        {dayBadge.text}
                      </span>
                      <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${isFull ? 'bg-red-50 text-red-600 border border-red-200' : isAlmostFull ? 'bg-orange-50 text-orange-600 border border-orange-200' : 'bg-green-50 text-green-700 border border-green-200'
                        ...
                        {isFull ? t('fullCapacity', 'FULL CAPACITY') : `${slot.max_capacity - slot.current_bookings} ${t('seatsLeft', 'SEATS LEFT')}`}
                      </span>
                    </div>

                    <h4 className="text-lg font-extrabold text-gray-900 mt-1">
                      {dateObj.toLocaleDateString(getLocaleTag(), { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}
                    </h4>
                    <div className="flex items-center gap-2 text-gray-600 text-sm mt-1 font-medium">
                      <Clock className="w-4 h-4 text-blue-600" />
                      <span>{dateObj.toLocaleTimeString(getLocaleTag(), { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>

                    <div className="mt-4 pt-3 border-t border-gray-100">
                      <div className="flex justify-between text-xs font-mono text-gray-500 mb-1">
                        <span>{t('capacityQuota', 'Capacity Quota')}</span>
                  ...
                  >
                    {bookingInProgress === slot.id ? (
                      <>
                        ...
                        <span>{t('lockingRow', 'Locking Row...')}</span>
                      </>
                    ) : isAlreadyBooked ? (
                      <>
                        <CheckCircle className="w-4 h-4" />
                        <span>{t('alreadyReserved', 'Already Reserved')}</span>
                      </>
                    ) : isFull ? (
                      <span>{t('slotFull', 'Slot Full')}</span>
                    ) : (
                      <span>{t('reserveThisSlot', 'Reserve This Slot')}</span>
                    )}
                  </button>
                </div>
              );
